...
export const createCurrencyExchangeTrpcRoute = currencyExchangeCreateProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    if (
      ...
      input.minTransactionAmountInSelectedCurrency > input.amountInSelectedCurrencyTo
    ) {
      throw new Error(
        'Minimal transaction amount must be lower than amount (in selected currency)'
      );
    }
