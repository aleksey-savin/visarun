...
export const editCurrencyExchangeTrpcRoute = currencyExchangeUpdateProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    const transactionSum = existingExchange.transactions.reduce((acc: number, curr) => {
      const amount = curr.amountInSelectedCurrency;
      return acc + (amount ? Number(amount) : 0);
    }, 0);

    if (input.amountInSelectedCurrencyTo && input.amountInSelectedCurrencyTo < transactionSum) {
      throw new Error(
        'Transactions sum must be smaller than exchange amountTo (in selected currency)'
      );
    }

    // Minimal transaction amount validation (must be lower than amountTo)
    const minTransactionAmountInSelectedCurrency =
      input.minTransactionAmountInSelectedCurrency ||
      (existingExchange.minTransactionAmountInSelectedCurrency
        ? Number(existingExchange.minTransactionAmountInSelectedCurrency)
        : null);
    if (
      minTransactionAmountInSelectedCurrency &&
      input.amountInSelectedCurrencyTo &&
      minTransactionAmountInSelectedCurrency > input.amountInSelectedCurrencyTo
    ) {
      throw new Error(
        'Minimal transaction amount must be lower than amountTo (in selected currency)'
      );
    }
