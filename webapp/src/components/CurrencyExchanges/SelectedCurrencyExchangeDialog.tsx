...
const SelectedCurrencyExchangeDialog = ({
  ...
  const selectedCurrencyExchange = allCurrencyExchanges.find(
    ex => ex.id === selectedCurrencyExchangeId
  );
  ...
  const finalTransactionForBegotteningExchange = selectedCurrencyExchange?.isBegottening
    ? selectedCurrencyExchange.transactions.find(
        tr =>
          Number(tr.amountInSelectedCurrency) ===
            Number(selectedCurrencyExchange.amountInSelectedCurrencyTo) &&
          Number(tr.amountInSelectedCurrency)
      )
    : null;
    ...
    try {
      ...
      const newTransaction = {
        ...
        begottenByCurrencyExchangeId:
          updatedTransactionData?.transaction?.begottenByCurrencyExchangeId || undefined,
        currencyExchangeId: updatedTransactionData?.transaction?.currencyExchangeId,
        amountInSelectedCurrency: Number(
          updatedTransactionData?.transaction?.amountInSelectedCurrency
        ),
        checkUrl: updatedTransactionData?.transaction?.checkUrl,
        senderId: updatedTransactionData?.transaction?.senderId || undefined,
      ...
      const exchangeWhereTransactionIs = allCurrencyExchanges.find(
        ex => ex.id === newTransaction.currencyExchangeId
      );
      ...
      const begotteningExchange = allCurrencyExchanges.find(
        ex => ex.id === newTransaction.begottenByCurrencyExchangeId
      );
  ...
  const handleAddTransaction = async () => {
    try {
      ...
      const newTransaction = {
        ...
        begottenByCurrencyExchangeId:
          newTransactionData?.transaction?.begottenByCurrencyExchangeId || undefined,
        currencyExchangeId: newTransactionData?.transaction?.currencyExchangeId,
        amountInSelectedCurrency: Number(newTransactionData?.transaction?.amountInSelectedCurrency),
        ...
        senderId: newTransactionData?.transaction?.senderId || undefined,
      };
  ...
  const handleEditTransaction = async (transaction: StoreTransaction) => {
    try {
      ...
      const newTransaction = {
        ...
        begottenByCurrencyExchangeId:
          editedTransactionData?.transaction?.begottenByCurrencyExchangeId || undefined,
        currencyExchangeId: editedTransactionData?.transaction?.currencyExchangeId,
        amountInSelectedCurrency: Number(
          editedTransactionData?.transaction?.amountInSelectedCurrency
        ),
        checkUrl: editedTransactionData?.transaction?.checkUrl,
        senderId: editedTransactionData?.transaction?.senderId || undefined,
        status: editedTransactionData?.transaction?.status,
        isCompanyTransaction: editedTransactionData?.transaction?.isCompanyTransaction || false,
        isInCash: editedTransactionData?.transaction?.isInCash || false,
      };
  ...
  const handleDeleteTransaction = async (id: string) => {
    try {
      ...
      const exchangeWhereTransactionWas = allCurrencyExchanges.find(ex =>
        ex.transactions.some(tr => tr.id === id)
      );
      ...
      const begotteningExchange = allCurrencyExchanges.find(ex =>
        ex.begottenTransactions.some(tr => tr.id === id)
      );
      if (begotteningExchange) {
        // if found -> transaction was begotten, and we must update begottening exchange too

        const newBegottenTransactions = begotteningExchange.begottenTransactions.filter(
          tr => tr.id !== id
        );
        ...
        setAllCurrencyExchanges([
          ...allCurrencyExchanges.filter(
            ex => ex.id !== exchangeWhereTransactionWas.id && ex.id !== begotteningExchange.id
          ),
  ...
  const handleAddFinalTransactionForBegotteningExchange = async () => {
    try {
      ...
      const newTransaction = {
        ...
        status: newTransactionData?.transaction?.status,
        isCompanyTransaction: newTransactionData?.transaction?.isCompanyTransaction,
        isInCash: newTransactionData?.transaction?.isInCash || false,
        senderId: newTransactionData?.transaction?.senderId || undefined,
      };
            ...
            <div className="flex flex-col gap-5">
                        ...
                        <div className="flex min-w-1/4 flex-col justify-center pt-1 gap-0.5">
                          <div className={`text-white flex gap-25 justify-between text-sm`}>
                            <span>
                              {getCurrencyAmount(
                                selectedCurrencyExchange.amountInSelectedCurrencyFrom,
                                selectedCurrencyExchange.fromCurrency.name
                              )}
                            </span>
                            <span>
                          ...
                          <Progress
                            value={
                              (selectedCurrencyExchange.begottenTransactions
                ...
                {selectedCurrencyExchange?.isBegottening ? (
                  ...
                  <div>
                    {(selectedCurrencyExchange?.finishedTransactionsAmountInSelectedCurrency ||
                      selectedCurrencyExchange?.finishedTransactionsAmountInSelectedCurrency ===
                        0) &&
                      (selectedCurrencyExchange?.inProgressTransactionsAmountInSelectedCurrency ||
                        selectedCurrencyExchange?.inProgressTransactionsAmountInSelectedCurrency ===
                          0) &&
                      selectedCurrencyExchange?.amountInSelectedCurrencyTo &&
                      selectedCurrencyExchange?.toCurrency && (
              ...
              {!selectedCurrencyExchange?.isBegottening && (
                <div className="flex gap-2 text-sm text-muted-foreground">
                  <div>
                    Dl:{' '}
                    {selectedCurrencyExchange?.deadline
                      ? formatDate(selectedCurrencyExchange.deadline)
                      : 'Indefinitely'}
                  </div>
                  <div>
                    Min Pymt:{' '}
                    {selectedCurrencyExchange?.minTransactionAmountInSelectedCurrency ||
                      'Indefinitely'}
                  </div>
                </div>
              )}
                ...
                finalTransactionForBegotteningExchange ? (
                  <div className="flex">
                    <span className="text-xs">
                      Payment for {selectedCurrencyExchange?.orderItem?.client?.firstName}{' '}
                      {selectedCurrencyExchange?.orderItem?.client?.lastName}
                    </span>
                  </div>
                ) : (
                  <div className="flex" onClick={() => setSummaryIsHidden(prevState => !prevState)}>
                    <span className="text-xs">
                      Payment summary for {selectedCurrencyExchange?.orderItem?.client?.firstName}{' '}
                      {selectedCurrencyExchange?.orderItem?.client?.lastName}
                    </span>
                    {summaryIsHidden ? (
                  ...
                  ? selectedCurrencyExchange?.begottenTransactions?.map(transaction => {
                      return (
                        <TransactionCard
                          ...
                          exchangeDetailsToShow={allCurrencyExchanges.find(
                            ex => ex.id === transaction.currencyExchangeId
                          )}
                  ...
                  : selectedCurrencyExchange?.transactions?.map(transaction => {
                      return (
                        <TransactionCard
                          ...
                          transaction={transaction}
                          exchangeDetailsToShow={allCurrencyExchanges.find(
                            ex => ex.id === transaction.begottenByCurrencyExchangeId
                          )}
                          handleDeleteTransaction={handleDeleteTransaction}
                          handleEditTransaction={handleEditTransaction}
                          copiedText={copiedText}
