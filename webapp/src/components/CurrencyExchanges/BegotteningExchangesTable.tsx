...
import {AlertCircle, Check, Crown, Eye, Loader} from "lucide-react";
                                    ...
                                    <div className="flex gap-3 w-full justify-between">
                                        <div className="w-full flex flex-col justify-center">
                                            <div className={`${
                                                ...
                                                <span>
                                                    {getCurrencyAmount(exchange.amountInSelectedCurrencyFrom)}
                                                </span>
                                                <span>
                                                    {getCurrencySymbol(exchange.fromCurrency.name)}
                                                </span>
                                            </div>

                                            <Progress
                                                value={
                                                    (exchange.begottenTransactions.filter(tr => tr.status === 'completed').reduce((acc, tr) => acc + Number(tr.amountInSelectedCurrency), 0) / exchange.amountInSelectedCurrencyFrom * 100) || 0
                                                }
                            ...
                            {(
                                ...
                                ? (
                                    <Summ className={`${exchange.status === 'finished' ? 'bg-muted text-gray-500' : 'bg-emerald-900'}`}>
                                        <Check />
                                        {formatCurrency(exchange.begottenTransactions.filter(tr => tr.status === 'completed').reduce((acc, tr) => acc + Number(tr.amountInSelectedCurrency), 0), exchange.fromCurrency.name)}
                                    </Summ>
                                ) : 'N/A'
                            }
                        </TableCell>
                        <TableCell>
                            <Summ className={`${exchange.status === 'finished' ? 'bg-muted text-gray-500' : 'bg-amber-700'}`}>
                                <Loader />
                                {formatCurrency(exchange.begottenTransactions.filter(tr => tr.status !== 'completed').reduce((acc, tr) => acc + Number(tr.amountInSelectedCurrency), 0), exchange.fromCurrency.name)}
                            </Summ>
                        </TableCell>
