...
const AllCurrencyExchangesPage = () => {
  ...
  const {
    ...
    isLoading: isLoadingAllExchanges,
    error: errorAllExchanges,
  } = trpc.currencyExchange.getAll.useQuery({});
  ...
  useEffect(() => {
    ...
    setAllCurrencyExchanges(
      allCurrencyExchangesData?.currencyExchanges?.map((exchange: any) => ({
        ...exchange,
        ...
        transactions:
          exchange.transactions?.map((transaction: any) => ({
            ...transaction,
        ...
        begottenTransactions:
          exchange.begottenTransactions?.map((transaction: any) => ({
            ...transaction,
            amountInSelectedCurrency: transaction.amountInSelectedCurrency
              ? Number(transaction.amountInSelectedCurrency)
              : null,
    ...
    <div className="grid gap-6 p-6">
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 md:gap-4 sm:items-end">
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 md:gap-4 flex-1 min-w-0">
          <Select value={selectedCombination} onValueChange={handleSelectCombination}>
